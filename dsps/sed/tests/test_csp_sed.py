...
def test_calc_rest_sed_wrapper():
    ran_key = jran.PRNGKey(SEED)
    t_obs = 13.0
    n_t = 500
    gal_t_table = np.linspace(T_BIRTH_MIN, t_obs, n_t)
    ...
    n_wave = 20
    ssp_flux = jran.uniform(sed_key, minval=0, maxval=1, shape=(n_met, n_ages, n_wave))
    ssp_wave = np.linspace(100, 1e5, n_wave)
    ssp_data = ssp_lgmet, ssp_lg_age_gyr, ssp_wave, ssp_flux

    gal_lgmet_data = gal_lgmet_table, gal_lgmet_scatter

    sed_info = calc_rest_sed(
        gal_t_table,
        gal_sfr_table,
        gal_lgmet_data,
        ssp_data,
        t_obs,
        lgmet_model="lgmet_table",
    )
    rest_sed, weights, lgmet_weights, age_weights = sed_info
