...
def test_params_invert():
    ...
    for __ in range(n_tests):
        ...
        p = fb._get_params_from_u_params(u_p)
        u_p2 = fb._get_u_params_from_params(p)
...
def test_age_weights_is_finite_and_zero_for_edge_cases():
    ...
    age_weights = fb._age_weights_from_params(lgyr_test, (lgyr_peak, lgyr_max))
...
def test_age_weights_from_params_scales_with_lgyr_max_as_expected():
    ...
    for __ in range(n_tests):
        ...
        age_weights = fb._age_weights_from_params(lgyr, params)
        zmsk = lgyr > params[1]
        assert np.all(age_weights[zmsk] == 0)
        assert np.any(age_weights > 0)

        age_weight_at_lgyr_peak = fb._age_weights_from_params(lgyr_peak, params)
        assert age_weight_at_lgyr_peak > 0

        lgyr_post_peak = np.linspace(lgyr_peak, lgyr_peak + 1, 10)
        age_weights_post_peak = fb._age_weights_from_params(lgyr_post_peak, params)
        assert np.all(np.diff(age_weights_post_peak) < 0)

    lgyr_peak = 5.5

    lgyr_test = np.linspace(lgyr_peak, lgyr_peak + 1, 100)
    age_weight_younger = fb._age_weights_from_params(lgyr_test, (lgyr_peak, 6.5))
    age_weights_older = fb._age_weights_from_params(lgyr_test, (lgyr_peak, 8.0))
...
def test_age_weights_from_default_params_are_weights():
    ...
    age_weights = fb._age_weights_from_params(lgyr, fb.DEFAULT_PARAMS)
    assert np.all(np.isfinite(age_weights))
...
def test_age_weights_from_random_params_are_weights():
    ...
    for __ in range(n_tests):
        u_p = np.random.uniform(-10, 10, 2)
        age_weights = fb._age_weights_from_u_params(lgyr, u_p)
        assert np.all(np.isfinite(age_weights))
        assert np.all(age_weights >= 0)
        assert np.any(age_weights > 0)
        assert np.allclose(age_weights.sum(), 1.0, rtol=1e-3)
...
def test_age_weights_from_random_params_u_params_consistency():
    ...
    for __ in range(n_tests):
        ...
        age_weights = fb._age_weights_from_u_params(lgyr, u_p)
        p = fb._get_params_from_u_params(u_p)
        age_weights2 = fb._age_weights_from_params(lgyr, p)
...
def test_compute_bursty_age_weights_from_params():
    ...
    bursty_age_weights = fb._compute_bursty_age_weights_from_params(
        lgyr_since_burst, age_weights, fburst, fb.DEFAULT_PARAMS
    )
...
def test_compute_bursty_age_weights_from_u_params():
    ...
    for __ in range(n_tests):
        ...
        age_weights = age_weights / age_weights.sum()
        fburst = np.random.uniform(0, 1)
        bursty_age_weights = fb._compute_bursty_age_weights_from_u_params(
        ...
        p = fb._get_params_from_u_params(u_p)
        bursty_age_weights2 = fb._compute_bursty_age_weights_from_params(
